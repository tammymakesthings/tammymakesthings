...
(defprotocol Markup
  ...
  an ext(ension) for finding markup file names, and a render-fn that returns a
  fn with the signature [java.io.Reader config] -> String (HTML)."
  (dir [this])
  (ext [this])
  (render-fn [this]))

(defn rewrite-hrefs
  "Injects the blog prefix in front of any local links ex. <img
  src='/img/cryogen.png'/> becomes <img src='/blog/img/cryogen.png'/>"
  [blog-prefix text]
...
(defn markups
  ...
  for a client of this ns. This vector should be used to iterate over
  supported Markups."
  []
  @markup-registry)
